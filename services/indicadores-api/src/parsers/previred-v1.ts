...
export class ParseError extends Error {
  constructor(
    ...
    super(
      `${source}: parse failed — missing fields: ${missingFields.join(", ")}`,
    );
...
function extractValueNumber(text: string): number | null {
...
function titleCaseAfp(name: string): string {
  if (name === "planvital") return "PlanVital";
  if (name === "uno") return "Uno";
  return name.charAt(0).toUpperCase() + name.slice(1);
}

function extractAfpRates(lines: string[]): AfpRate[] {
  const rates: AfpRate[] = [];
  for (const line of lines) {
    const nl = norm(line);
    if (!nl.includes("afp")) continue;
    for (const name of AFP_NAMES) {
      if (!nl.includes(name)) continue;
...
function extractCesantiaRates(lines: string[]): CesantiaRate[] {
  const rates: CesantiaRate[] = [];
  for (const line of lines) {
    const nl = norm(line);
    if (nl.includes("tope")) continue; // skip the topes section
...
function extractAsignacionFamiliar(lines: string[]): AsignacionFamiliarTramo[] {
  const tramos: AsignacionFamiliarTramo[] = [];
  for (const line of lines) {
    const nl = norm(line);
    if (!nl.includes("tramo")) continue;
    for (const letter of TRAMO_LETTERS) {
      if (!nl.includes(`tramo ${letter}`)) continue;
      const numbers = extractNumbers(line);
      if (numbers.length < 2) continue;
 ...
 */
export function parsePreviredV1(
  text: string,
  options: ParsePreviredOptions,
): PreviredData {
  const lines = splitLines(text);
  const fallback = options.fallback ?? {};

  const result: Partial<PreviredData> = {};
  const missing: string[] = [];

  const read = (
    field: keyof PreviredData,
    labels: string[],
    source?: number | null,
  ): void => {
  ...
  if (afpRates.length === 0) missing.push("afpRates");
  else result.afpRates = afpRates;

  const cesantiaRates = extractCesantiaRates(lines);
  if (cesantiaRates.length === 0) missing.push("cesantiaRates");
  else result.cesantiaRates = cesantiaRates;

  const asignacionFamiliar = extractAsignacionFamiliar(lines);
  if (asignacionFamiliar.length === 0) missing.push("asignacionFamiliar");
  else result.asignacionFamiliar = asignacionFamiliar;

  if (missing.length > 0) {
    throw new ParseError(missing, "previred");
  }

  return result as PreviredData;
}
