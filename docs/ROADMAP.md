...
- [ ] **Bug Decimal en `/dashboard/trabajadores`** (la lista, no la ficha): "Only plain objects can be passed to Client Components... Decimal objects are not supported" (`app/dashboard/trabajadores/page.tsx:66`). Serializar `sueldoBase` (y cualquier Decimal) a number antes de pasar `workers` a `WorkersClientList`. Bug preexistente, no relacionado a nada de esta sesión.
