...
- [ ] **Template de contrato completo (10 cláusulas) + datos personales** — 2026-08-27 implementado localmente. Nuevo PDF legal (art. 10 Código del Trabajo) con datos reales: fecha nacimiento, nacionalidad, estado civil, profesión, domicilio, comuna, ciudad y correo del trabajador; correo de la empresa; forma de pago, periodicidad, minutos de colación y comuna del lugar de trabajo en el contrato. Incluye `lib/numero-en-palabras.ts` (monto en palabras). **PENDIENTE CRÍTICO: correr `prisma/migrations/add_contract_personal_data.sql` en Supabase ANTES de desplegar** (los campos nuevos no existen en BD; sin el SQL, el deploy rompe con P2022).
