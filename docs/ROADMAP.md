...
- [x] **Template de contrato completo (10 cláusulas) + datos personales** — 2026-08-27/28. Nuevo PDF legal (art. 10 Código del Trabajo) con datos reales: fecha nacimiento, nacionalidad, estado civil, profesión, domicilio, comuna, ciudad y correo del trabajador; correo de la empresa; forma de pago, periodicidad, minutos de colación y comuna del lugar de trabajo en el contrato. Incluye `lib/numero-en-palabras.ts` (monto en palabras). SQL `add_contract_personal_data.sql` ejecutado en Supabase y pusheado (`a161376`).
- [x] **Contrato: QUINTO sin forma de pago duplicada + cláusula de gratificación** — 2026-08-28 (`791c2d8`). QUINTO ya no repite "mediante Transferencia bancaria" (queda solo en CUARTO). Gratificación inteligente según `Worker.tipoGratificacion`: pactada → monto; legal (default) → "25% de la remuneración mensual con tope de 4,75 ingresos mínimos mensuales, conforme artículos 47 y 50 del Código del Trabajo".
- [x] **Fix build: Resend lazy init** — 2026-08-27 (`f818cad`). `new Resend()` movido de top-level a inicialización lazy dentro de `sendAlertEmail` (después del guard): sin `RESEND_API_KEY` el build ya no falla en "Collecting page data for /api/admin/indicadores/sync". Antes, cualquier deploy sin la key moría desde la Fase 2 (b590712).
